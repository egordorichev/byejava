...
using org.rexcellentgames.byejava.scanner;
using java.util;

namespace org.rexcellentgames.byejava.parser {
	public class Parser {
		private ArrayList<Token> tokens;
		private String code;
		private int current;
		public Parser(ArrayList tokens, String code) {
			this.setTokens(tokens, code);
		}

		public void setTokens(ArrayList tokens, String code) {
			this.tokens = tokens;
			this.code = code;
			this.current = 0;
		}
		...
		private Token peekNext() {
			return this.current > this.tokens.size() - 2 ? this.tokens.get(this.tokens.size() - 1) : this.tokens.get(this.current + 1);
		}
		...
		private ArrayList parseVarGenerics<Generetic> (boolean consumed) {
		...
		private Expression parseMultiplication() {
			...
			while (this.match(TokenType.STAR, TokenType.SLASH, TokenType.PERCENT, TokenType.INSTANCEOF)) {
				TokenType operator = this.peekPrevious().type;
		...
		private Expression parseAssignment() {
			...
			if this.match(TokenType.EQUAL) {
				...
				if expression is Expression.Variable || expression is Expression.Index {
		...
		private Statement parseClassStatement(Modifier modifier) {
			...
			while (!this.match(TokenType.RIGHT_BRACE)) {
				...
				if statement != null {
					if statement is Statement.Class || statement is Statement.Enum {
						if inner == null {
							inner = new ArrayList<>();
						} 
						inner.add(statement);
					} else if statement is Statement.Method {
						if methods == null {
		...
		private Statement parseDeclaration(Modifier modifier) {
			...
			switch (type) {
				case CLASS: {
					return parseClassStatement(modifier == null ? new Modifier() : modifier);
				}

				case ENUM: {
					return parseEnumStatement(modifier == null ? new Modifier() : modifier);
				}
		...
		private void tryAdd(ArrayList statements, Statement statement) {
			if statement != null {
				statements.add(statement);
			} 		}

		public ArrayList parse<Statement> () {
